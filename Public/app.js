...
document.getElementById('btnConnect').addEventListener('click', () => {
  ...
  client.on('reconnect', () => statusEl.textContent = 'Status: reconectando...');
  client.on('error', err => {
    statusEl.innerHTML = 'Status: <em style="color:red">erro</em>';
    console.error('MQTT Erro:', err);
  });
  client.on('offline', () => statusEl.textContent = 'Status: offline');
...
function refreshCard(meta, raw) {
  ...
  if (isBit) {
    ...
    // remove e aplica só as classes corretas
    ...
    pVal.className = 'val';
    pVal.textContent = value.toFixed(divisor>1?2:0) + (unit?` [${unit.toUpperCase()}]`:'');
